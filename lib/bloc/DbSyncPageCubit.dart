...
class DbSyncPageCubit extends Cubit<DbSyncPageState> {
  ...
  DbSyncPageCubit(this.globalProvider) : super(DbSyncPageInitial());
  ...
  void startSyncProcess(String token) async {
    ...
    if (syncCnt > 0) {
      ...
      if (token.isEmpty) {
        try {
          deviceRegistered = true; //TODO await _srv.checkRegistrationStatus(_phoneId);
        } catch (e) {
          if (!deviceRegistered) {
      ...
      if (deviceRegistered) {
        ...
        if (companyInfCheckbox > 0) {
          ...
          try {
            ...
            dbService?.getCompany().then((respDkCompanyInfo) async {
        ...
        if (resourcesCheckBox == 1) {
          ...
          try {
            ...
            if (pStatus.isGranted) {
              dbService?.getResCategories().then((respDkCategories) async {
                ...
                if (respDkCategories.isNotEmpty) {
                  ...
                  //   }
                  // }).toList();
                  await DbHelper.insertCategoryList('tbl_dk_res_category', respDkCategories);
                }
                syncCnt = syncCnt - 1;
              }).catchError((_) {
                resourcesCheckBox = 4;
          ...
          try {
            ...
            dbService?.getResources().then((respDkResources) async {
        ...
        if (resPricesCheckBox == 1) {
          ...
          try {
            dbService?.getResPriceGroup().then((respDkResPriceGroupList) async {
          ...
          //region ResPrices
          try {
            dbService?.getResPrices().then((respDkPrices) async {
        ...
        if (imagesCheckBox == 1) {
          ...
          try {
            ...
            if (pStatus.isGranted) {
              ...
              while (offset >= 0) {
                List<TblDkImage> respDkImages = await dbService!.getImages(img, offset, 10);
                try {
